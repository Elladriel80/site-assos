...
| … | `admin_migration_phase21_declaration_tablette.sql` | Phase 21 — Déclaration à la tablette | RPC `declare_battle()` et `acclaim_opponent()` appelables en anonyme, avec garde-fous en dur (une partie par paire et par ronde, 3 parties max par champion et par ronde, 30 s de battement). Colonne `battles.source`, lien acclamation ↔ bataille sur `royal_votes`, fonction `current_round()` lue depuis `campaign_state`. **Supprime la policy « Anon can read battles »** qui exposait les emails réels des joueurs, remplacée par la vue `battles_public` sans email |
